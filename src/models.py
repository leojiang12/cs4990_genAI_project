...
class UNetGenerator(nn.Module):
    def __init__(self, in_ch=3, out_ch=3, features=64):
        ...
        self.down2 = nn.Sequential(nn.Conv2d(features, features*2, 4, 2, 1), nn.BatchNorm2d(features*2), nn.LeakyReLU(0.2, inplace=False))
        # … add more downs as desired …
        # Decoder: upsampling
        self.up1   = nn.Sequential(nn.ConvTranspose2d(features*2, features, 4, 2, 1), nn.BatchNorm2d(features), nn.ReLU(inplace=False))
        # Final output
        self.final = nn.Sequential(nn.ConvTranspose2d(features, out_ch, 4, 2, 1), nn.Tanh(inplace=False))
...
class PatchDiscriminator(nn.Module):
    def __init__(self, in_ch=6, features=64):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(features, features*2, 4, 2, 1), nn.BatchNorm2d(features*2), nn.LeakyReLU(0.2, inplace=False),
